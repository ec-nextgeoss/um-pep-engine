...
Included with the PEP there is a script at the source path that performs queries against a Mongo Database. The main purpose of this script is to reduce the usage of RAM when registering a resource and when querying for its content.
...
* **get_id_from_uri**: This feature will return the id for the best candidate of the match by a URL given.
* **delete_resource**: This function will recive a resource id and will find and delete the matched document

This script is manipulated by the API which would intercept the request in order to perform GET,PUT,POST and DELETE methods.
